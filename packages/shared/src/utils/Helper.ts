...
export function formatPercentDelta(delta: number, defaultSign = "+"): string {
   return `${mathSign(delta, defaultSign)}${formatPercent(Math.abs(delta))}`;
}
...
export function mapNotNull<T, K>(iterable: Iterable<T>, func: (value: T) => K): NonNullable<K>[] {
...
export function monthsBetween(from: Date, to: Date): number {
   return Math.abs((from.getFullYear() - to.getFullYear()) * 12 + (from.getMonth() - to.getMonth()));
}

export function fallbackText(text: string | undefined | null, fallback: string): string {
