...
export class Layout {
   ...
   public hexToPixel(h: Hex): IPoint {
   ...
   public pixelToHex(p: IPoint): Hex {
      const M: Orientation = this.orientation;
      const size: IPoint = this.size;
      const origin: IPoint = this.origin;
      const pt: IPoint = { x: (p.x - origin.x) / size.x, y: (p.y - origin.y) / size.y };
      const q: number = M.b0 * pt.x + M.b1 * pt.y;
      const r: number = M.b2 * pt.x + M.b3 * pt.y;
      return new Hex(q, r, -q - r);
   }

   public hexCornerOffset(corner: number): IPoint {
      const M: Orientation = this.orientation;
      const size: IPoint = this.size;
   ...
   public polygonCorners(h: Hex): IPoint[] {
   ...
   public polygonCorner(h: Hex, direction: number): IPoint {
      const center: IPoint = this.hexToPixel(h);
      const offset: IPoint = this.hexCornerOffset(direction);
