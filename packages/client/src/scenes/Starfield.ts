...
export class Starfield extends Container {
   ...
   constructor() {
      ...
      this.watermark = G.scene.overlay.addChild(
         new BitmapText("", { fontName: Fonts.MainFont, fontSize: 12, tint: 0x999999, align: "right" }),
      );
   ...
   public update(): void {
      ...
      this.watermark.text = `TICK: ${G.save.state.tick}    FPS: ${Math.round(this.fps.reduce(sum, 0) / this.fps.size)}    VERSION: ${this.version}    ${navigator.onLine ? "ONLINE" : "OFFLINE"}`;
