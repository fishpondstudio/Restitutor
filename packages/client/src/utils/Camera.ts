import { clamp, lerp, sizeOf, type ValueOf } from "@project/shared/src/utils/Helper";
import { type IHaveXY, lerpVector2, v2 } from "@project/shared/src/utils/Vector2";
import type { Application, FederatedPointerEvent, IDestroyOptions } from "pixi.js";
import { Container, Rectangle } from "pixi.js";
import { EdgePanMovement, type IEdgePanOptions } from "./EdgePanMovement";
...
export type WheelMode = ValueOf<typeof WheelMode>;
...
export class Camera extends Container implements SceneLifecycle {
   constructor(
      ...
      this.edgePanMovement = new EdgePanMovement(this, app.renderer.events.domElement, options.edgePanOptions);
   ...
   private edgePanMovement: EdgePanMovement;
   ...
   private onPointerUp(e: FederatedPointerEvent) {
      ...
      if (p && !p.moved) {
         e.detail = 1;
         if (e.button === 0) {
            if (this.lastPointerUpPosition) {
               const dx = e.x - this.lastPointerUpPosition.x;
               const dy = e.y - this.lastPointerUpPosition.y;
               if (dx ** 2 + dy ** 2 < 10 && Date.now() - this.lastPointerUpAt < 500) {
                  e.detail = 2;
               }
            }
            this.lastPointerUpAt = Date.now();
            this.lastPointerUpPosition = { x: e.x, y: e.y };
         }
         this.emit("clicked", e);
      }
   }
   ...
   public screenToWorld(pos: IHaveXY): IHaveXY {
      return {
   ...
   public worldToScreen(pos: IHaveXY): IHaveXY {
      return {
   ...
   public wheelMode: WheelMode = WheelMode.Zoom;
   ...
   private targetOrigin: IHaveXY | null = null;
   private cursorPos: IHaveXY | null = null;
   ...
   private update = () => {
      ...
      this.edgePanMovement.update(this.app.ticker.deltaMS / 1000);
      if (this.targetZoom) {
      ...
      switch (this.wheelMode) {
         case WheelMode.Zoom: {
            ...
            this.targetZoom = clamp(
               this.zoom - e.deltaY * 0.001 * this.options.scrollSensitivity() * this.zoom,
               this.minZoom,
               this.maxZoom,
            );
            break;
         ...
         case WheelMode.HorizontalScroll: {
            this.targetOrigin = v2(this.pivot).addSelf({ x: e.deltaY * 2 * this.options.scrollSensitivity(), y: 0 });
            break;
         }
         case WheelMode.VerticalScroll: {
            this.targetOrigin = v2(this.pivot).addSelf({ x: 0, y: e.deltaY * 2 * this.options.scrollSensitivity() });
            break;
         }
      }
   };
   ...
   public set center(point: IHaveXY) {
      this.targetOrigin = null;
      this.moveOrigin(this.centerToOrigin(point));
   }

   public clampCenter(point: IHaveXY, result?: IHaveXY): IHaveXY {
      return this.originToCenter(this.clampOrigin(this.centerToOrigin(point, result), result), result);
   }

   public centerToOrigin(point: IHaveXY, result?: IHaveXY): IHaveXY {
   ...
   public originToCenter(point: IHaveXY, result?: IHaveXY): IHaveXY {
      const x = point.x + this.screenWidth / this.scale.x / 2;
      const y = point.y + this.screenHeight / this.scale.y / 2;
      if (result) {
         result.x = x;
   ...
   public clampOrigin(point: IHaveXY, result?: IHaveXY): IHaveXY {
   ...
   public onDisable(): void {
      ...
      this.edgePanMovement.onDisable();
