...
import { ChatFlag, type IChat, type Language } from "@project/shared/src/rpc/ServerMessageTypes";
import { cls, hasFlag, mapOf } from "@project/shared/src/utils/Helper";
import { TypedEvent } from "@project/shared/src/utils/TypedEvent";
import { memo, useCallback, useEffect, useRef, useState } from "react";
import Mod from "../assets/images/Mod.png";
import { GameOptionUpdated, OnChatMessage } from "../game/Events";
import { GameOptionFlag } from "../game/GameOption";
import { handleCommand } from "../game/HandleCommand";
import { Languages, LanguagesImage } from "../game/Languages";
import { getUser, useConnected } from "../rpc/HandleMessage";
...
const _SetChatInput = new TypedEvent<(oldChat: string) => string>();
...
function _ChatPanelSingle({ left, channel }: { left: number; channel: Language }): React.ReactNode {
   ...
   useTypedEvent(_OnImageLoaded, scrollToBottom);
   ...
   return (
      <div
         className={cls("chat-panel panel", isFocused ? "active" : null)}
         style={{ left }}
         onMouseEnter={() => {
            setIsHover(true);
         }}
...
function _ChatInput({
   ...
   const [isFocused, setIsFocused] = useState(false);
   useTypedEvent(_SetChatInput, (func) => {
      setMessage(func);
   });
   return (
...
const _OnImageLoaded = new TypedEvent<void>();
